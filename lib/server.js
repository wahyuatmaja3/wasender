...
app.all('/', async (req, res) => {
    ...
    if (!Penerima && !Pesan && !File && req.method === 'GET') {
    ...
    const cleanFile = File ? decodeURIComponent(File).replace(/\\{2,}/g, '\\') : '';

    // Masukkan ke antrian – response langsung tanpa menunggu dikirim
    const { id, position, estimatedWaitSec } = enqueue(Penerima, Pesan || '', cleanFile);
